...
class ProcessData:
    ...
    def extract_phys(self, df_raw):
        ...
        ## Microver 12/20/22
        ## We created separate lists to pull and store important data from raw data
        ...
        engine_sw_version_list = list()

        engine_fw_version_list = list()

        engine_serial_number_list = list()

        for idx, value in df_raw['DataBytes'].iteritems():
        
            ## if the length of the DataBytes in the raw data is greater than 64 bytes
            if(len(value)>64):
                
                can_id = df_raw["ID"][idx]

                engine_model_raw = self.remove_values_from_list(value[4:36], 255) # Removing nonsense characters and converting them to ascii characters

                engine_model = ''.join(chr(i) for i in engine_model_raw)

                engine_sw_version_raw = self.remove_values_from_list(value[36:68], 255)

                engine_sw_version = ''.join(chr(i) for i in engine_sw_version_raw)

                engine_fw_version_raw = self.remove_values_from_list(value[68:100], 255)

                engine_fw_version = ''.join(chr(i) for i in engine_fw_version_raw)

                engine_serial_number_raw = self.remove_values_from_list(value[100:132], 255)

                engine_serial_number = ''.join(chr(i) for i in engine_serial_number_raw)

                can_id_list.append(can_id)

                idx_list.append(idx)

                engine_model_list.append(engine_model)

                engine_sw_version_list.append(engine_sw_version)

                engine_fw_version_list.append(engine_fw_version)

                engine_serial_number_list.append(engine_serial_number)

        ## This side of lists created to make meaningfull csv output, I will use them in the final result dataframe

        list_can_id = list()

        list_time_stamp = list()
        ...
        for j in range(len(idx_list)):
            ...
            for i in range(4):
                ...
                list_can_id.append(can_id_list[j])

                list_time_stamp.append(idx_list[j])

                if(i == 0):

                    signal.append("EngineModel")

                    list_raw.append(engine_model_list[j])

                    list_physic.append(engine_model_list[j])

                elif(i == 1):

                    signal.append("EngineSwVersion")

                    list_raw.append(engine_sw_version_list[j])

                    list_physic.append(engine_sw_version_list[j])

                elif (i == 2):

                    signal.append("EngineFwVersion")

                    list_raw.append(engine_fw_version_list[j])

                    list_physic.append(engine_fw_version_list[j])

                elif(i == 3):

                    signal.append("EngineSerialnumber")

                    list_raw.append(engine_serial_number_list[j])

                    list_physic.append(engine_serial_number_list[j])

        df_ascii_v3 = pd.DataFrame()

        df_ascii_v3['TimeStamp'] = list_time_stamp

        df_ascii_v3['CAN ID'] = list_can_id

        df_ascii_v3['Signal'] = signal

        df_ascii_v3['Raw Value'] = list_raw

        df_ascii_v3['Physical Value'] = list_physic

        df_ascii_v3.set_index('TimeStamp', inplace=True)  ## Setting the timestamp column to index because this is important for application layer

        ## Extracting error messages from raw data
        df_raw = df_raw[df_raw['DataLength'] < 130]
...
class MultiFrameDecoder:
    ...
    def __init__(self, tp_type=""):
        ...
        if tp_type == "uds":
            ...
            self.frame_struct = frame_struct_ascii
